...
const main = async () => {
  ...
  const token1 = await Token.deploy("Tether", "USDT", toWei(100000000000000));
  ...
  const token3 = await Token.deploy("Dai", "DAO", toWei(100000000000000));
  await token3.deployed();
  const token3Symbol = await token3.symbol();
  console.log(`${token3Symbol} deployed to:`, token3.address);

  const token4 = await Token.deploy("Balancer", "BAL", toWei(100000000000000));
